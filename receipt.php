<?php
	...
	$conne -> init_conn();
	$select = "select o_id,o_price,o_time,g_name,g_imgurl,a_address,f_month,f_ratio,o_leave,count(*) from db_order,db_transaction,db_address,
		db_goods,db_fenqi where o_address=a_id and o_id=t_oid and g_id=t_gid and o_uid='".$_SESSION['C_USER']->mId."' and o_fid=f_id and 
		o_isshow='Y' and o_receive='N' group by t_oid";
...
<html>
	<head>
		...
		<title><?php echo $confige->getItem('account_title');?>-待收货</title>
	</head>
	<body>
		...
		<div class='account-main'>
			<div class='account-main-left cur-hand'>
				...
				<div id="account-main-left-receipt" class="account-text-l text-red cur-hand font17px">待收货</div>
			...
			<div class="account-main-mid">
				<div class="account-main-mid-receipt-header">
					<div id="all-order" class="order-item-text cur-hand mouse-on-orange-off-init"><strong>所有订单</strong></div>
					<div id="receipt-order" class="order-item-text text-orange cur-hand"><strong>待收货</strong></div>
					<div id="receive-order" class="order-item-text cur-hand mouse-on-orange-off-init"><strong>已收货</strong></div>
					<div id="fenqi-order" class="order-item-text cur-hand mouse-on-orange-off-init"><strong>分期进行时</strong></div>
					<a id="clear-all" style="float: right;margin-right: 20px;color: red;font-size: 16px;">一键清空</a>
				</div>
				<div class="account-main-mid-receipt-header-line">
					<div class="order-item-line-white"></div>
					<div class="order-item-line-orange"></div>
					<div class="order-item-line-white"></div>
					<div class="order-item-line-white"></div>
				</div>
							...
							<tbody>
								...
								<?php for($i=0;$i<count($array);$i++){?>
									<tr height="35px" valign="top" class="cur-hand">
										<td class="mouse-on-red" style="width:200px; text-align: left;line-height: 20px;padding: 5px;font-size: 12px;">
											<?php echo $array[$i]['g_name']."等".$array[$i]['count(*)']."件商品";?>
										</td>
										<td style="width:200px;text-align: center;height: 80px;line-height: 80px;">
											<?php echo $array[$i]['o_time'];?>
										</td>
										<td style="width:200px; text-align: center;height: 80px;color: orangered;line-height: 80px;">
											<?php echo $array[$i]['o_price'];?>
										</td>
										<td style="width:200px;text-align: center;height: 80px;line-height: 80px;">
											<?php echo $array[$i]['a_address'];?>
										</td>
									</tr>
								<?php }?>
							</tbody>
		...
		<script type="application/javascript">
			...
			$("#clear-all").click(function(){
				$.ajax({
					type: "POST",
					url: "./action/clearallfenqi.class.php",
					data: {
