...
if ($isSelf=="") {
	...
	$conne -> init_conn();
	$select = "select * from db_goods order by g_sellnumber desc limit ".(8+$p);
	...
	require_once '/db/conn.php';
	$conne -> init_conn();
	$select = "select * from db_goods where g_isself='Y' order by g_sellnumber desc limit ".(8+$p);
	$array = $conne -> getRowsArray($select);
	$result = array();
	for ($i = $p; $i < count($array); $i++) {
		$result[$i-$p] = array('mId' => $array[$i]['g_id'], 'mCid' => $array[$i]['g_cid'], 'mSid' => $array[$i]['g_sid'], 'mName' => $array[$i]['g_name'], 'mImgUrl' => $array[$i]['g_imgurl'], 'mPrice' => $array[$i]['g_price'], 'mSellNumber' => $array[$i]['g_number'], 'mNumber' => $array[$i]['g_sellnumber'], );
	...
	require_once './db/conn.php';
	$conne -> init_conn();
	$select = "select * from db_goods where g_isself='N' order by g_sellnumber desc limit ".(8+$p);
	$array = $conne -> getRowsArray($select);
	$result = array();
	for ($i = $p; $i < count($array); $i++) {
		$result[$i-$p] = array('mId' => $array[$i]['g_id'], 'mCid' => $array[$i]['g_cid'], 'mSid' => $array[$i]['g_sid'], 'mName' => $array[$i]['g_name'], 'mImgUrl' => $array[$i]['g_imgurl'], 'mPrice' => $array[$i]['g_price'], 'mSellNumber' => $array[$i]['g_number'], 'mNumber' => $array[$i]['g_sellnumber'], );
