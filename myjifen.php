<?php
	...
	$conne->init_conn();
	$select="select * from db_user where u_id='".$_SESSION['C_USER']->mId."'";
	$array=$conne->getRowsArray($select);
	$_SESSION['C_USER']->mJifen=$array[0]['u_jifen'];
	$conne->close_rst();
	$r="1";
	if(isset($_GET['r'])){
		$r=$_GET['r'];	
	...
	switch($r){
		case '1':
			$select1="select * from db_jifen_record";
			break;
		case '2':
			$select1="select * from db_jifen_record where jr_number>0";
			break;
		case '3':
			$select1="select * from db_jifen_record where jr_number<0";
			break;
	}
	$records=$conne->getRowsArray($select1);
	$conne->close_conn();	
?>
<!DOCTYPE html>
<html>
	<head>
		...
		<title><?php echo $confige->getItem('account_title');?>-我的积分</title>
	</head>
	<body>
		...
		<div class='account-main'>
			<div class='account-main-left cur-hand'>
				...
				<div id="account-main-left-myjifen" class="account-text-b text-red cur-hand font17px">我的积分</div>
			...
			<div class="account-main-mid">
				<div class="account-main-mid-jifen-top">
					<image style="height: 100px;width: 100px; float: left;margin-left: 10px;" src="./images/money.png"/>
					<aaa style="font-size: 14px;float: left;margin-top:-5px;margin-left: 20px;">欢迎您！<?php echo $_SESSION['C_USER']->mUserName;?></aaa><br/>
					<aaa style="font-size: 16px;float: left;margin-top:-20px;color:orangered;margin-left: 20px;">我的积分：</aaa>
					<aaa style="font-size: 35px;float: left;color:orangered;margin-top:-20px;"><strong><?php echo $_SESSION['C_USER']->mJifen;?></strong></aaa><br/><br/>
				</div>
				<div class="account-main-mid-jifen-header">
					<aaa id="suoyoujilu" class="account-main-mid-jifen-header-item-up cur-hand">所有记录</aaa>
					<aaa id="huoqujilu" class="account-main-mid-jifen-header-item-down cur-hand">获取记录</aaa>
					<aaa id="shiyongjilu" class="account-main-mid-jifen-header-item-down cur-hand">使用记录</aaa>
				</div>
				<div class="account-main-mid-jifen-record">
					<form>
						<table style="border: 1px solid #B7AEB4; width:720px; font-size:14px; margin-top:5px;" cellspacing="0" cellpadding="0">
							<tbody>
								<tr height="35px" valign="top" style="background: #DDDDDD;">
									<td style="width:200px; text-align: center;height: 30px; line-height: 30px;">
										时间
									</td>
									<td style="width:200px;text-align: center; height: 30px; line-height: 30px;">
										事件
								...
								<?php for($i=0;$i<count($records);$i++){?>
									<tr height="35px" valign="top" class="cur-hand">
										<td style="width:200px; text-align: center;height: 30px; line-height: 30px;">
											<?php echo $records[$i]['jr_time'];?>
										</td>
										<td style="width:200px;text-align: center; height: 30px; line-height: 30px;">
											<?php echo $records[$i]['jr_event'];?>
										</td>
										<td style="width:200px;text-align: center;color: orangered; height: 30px; line-height: 30px;">
											<?php 
												if($records[$i]['jr_number']>0){
													echo "+".$records[$i]['jr_number'];
												}else{
													echo $records[$i]['jr_number'];
												}
											?>
		...
		<?php
			...
			if($r=='1'){
				echo "<script type='text/javascript'>
				$('#suoyoujilu').attr('class', 'account-main-mid-jifen-header-item-up cur-hand');
				$('#huoqujilu').attr('class', 'account-main-mid-jifen-header-item-down cur-hand');
				$('#shiyongjilu').attr('class', 'account-main-mid-jifen-header-item-down cur-hand');
				</script>";
			}else if($r=='2'){
				echo "<script type='text/javascript'>
				$('#suoyoujilu').attr('class', 'account-main-mid-jifen-header-item-down cur-hand');
				$('#huoqujilu').attr('class', 'account-main-mid-jifen-header-item-up cur-hand');
				$('#shiyongjilu').attr('class', 'account-main-mid-jifen-header-item-down cur-hand');
				</script>";
			}else if($r==3){
				echo "<script type='text/javascript'>
				$('#suoyoujilu').attr('class', 'account-main-mid-jifen-header-item-down cur-hand');
				$('#huoqujilu').attr('class', 'account-main-mid-jifen-header-item-down cur-hand');
				$('#shiyongjilu').attr('class', 'account-main-mid-jifen-header-item-up cur-hand');
				</script>";
			}
		?>
