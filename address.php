...
<html>
	<head>
		...
		<title><?php echo $confige->getItem('homepage_title');?>-收货地址</title>
	</head>
	<body>
		<div class="cd-popup" role="alert">
			<div class="cd-popup-container" style="padding-top: 20px;">
				<aaa style="font-size: 20px;">添加地址</aaa>
				<div style="margin-top: 20px;margin-bottom: 20px;">
					姓名：<input id="input-name" type="text" /><br/>
					地址：<input id="input-address" type="text" /><br/>
					联系电话：<input id="input-phone" "text" /><br/>
					邮政编码：<input id="input-postcode" type="text" />
				</div>
				<ul class="cd-buttons">
					<li><a id="address-yes" class="cur-hand">确定</a></li>
					<li><a id="address-no" class="cur-hand">取消</a></li>
				</ul>
		...
		<div class='account-main'>
			<div class='account-main-left cur-hand'>
				...
				<div id="account-main-left-address" class="account-text-b text-red cur-hand font17px">收货地址设置</div>
			...
			<div class="account-main-mid">
				<div class="account-main-mid-top">
					<input class="account-main-mid-top-add-btn" onclick="submitOrder();" type="button" value="新增收货地址"></input>
					<aaa style="margin-left: 9px;">您已创建<a id="address-number"></a>个收货地址，最多可创建20个</aaa>
				</div>
				<div id="address-top" class="account-main-mid-address-top cur-hand" hidden>
					<div style="float:right ;margin-right: 10px;width: 30px;height: 70px;text-align: center;line-height: 70px;"><image src="./images/check_mark.png" style="height: 30px;width: 30px;margin-top: 20px;" ></image></div>
					<div class="address-top-top" style="font-size: 17px;height: 40px;text-align: center;">
						<aaa id="address-name" style="float: left;margin-left: 20px;height: 40px;line-height: 40px;">收件人：杨凯</aaa>
						<aaa id="address-phone" style="float: right;height:40px;line-height:40px;margin-right: 20px;">18940265355</aaa>
					</div>
					<div class="address-top-bottom" style="font-size: 13px;height: 30px;text-align: center;">
						<aaa id="address-value" style="float: left;margin-left: 20px;height: 30px;line-height: 30px;">收件地址：东北大学</aaa>
					</div>
				</div>
				<div id="address-list" class="account-main-mid-address-list">
					<?php for($i=0;$i<count($array);$i++){?>
						<div id="address-top" class="account-main-mid-address-list-item cur-hand" onclick="setDefault(<?php echo $array[$i]['a_id'];?>);">
							<div class="address-top-top" style="font-size: 17px;height: 40px;text-align: center;">
								<aaa id="address-name" style="float: left;margin-left: 20px;height: 40px;line-height: 40px;">收件人：<?php echo $array[$i]['a_name'];?></aaa>
								<aaa id="address-phoone" style="float: right;height:40px;line-height:40px;margin-right: 20px;"><?php echo $array[$i]['a_phone'];?></aaa>
							</div>
							<div class="address-top-bottom" style="font-size: 13px;height: 30px;text-align: center;">
								<aaa id="address-value" style="float: left;margin-left: 20px;height: 30px;line-height: 30px;">收件地址：<?php echo $array[$i]['a_address'];?></aaa>
							</div>
						</div>
					<?php }?>
				</div>
		...
		<script type="application/javascript">
			...
			<?php
				if(count($array)>0){
			?>
			$("#address-number").html(<?php echo count($array);?>);
			$("#address-top").removeAttr("hidden");
			$("#address-name").html('收件人：<?php echo $array[0]['a_name'];?>');
			$("#address-phone").html('<?php echo $array[0]['a_phone'];?>');
			$("#address-value").html('收件地址：<?php echo $array[0]['a_address'];?>');
			<?php
				}
			?>
			...
			$("#address-yes").click(function(){
		    	...
		    	$.ajax({
			         type: "POST",
			         url: "./action/addnewaddress.class.php",
			         data: {
			         	uid:<?php echo $_SESSION['C_USER']->mId;?>,
			         	name:$("#input-name").val(),
			...
			function setDefault(i){
				$.ajax({
					type: "POST",
					url: "./action/setmorenaddr.class.php",
					data: {
						aid:i
					},
					dataType: "json",
